...
namespace RadioOwlCmd.Helpers
{
    public class FileHelper
    {
        ...
        /// Nageruji vhodné filename dle části pořadu
        /// </summary>
        public string GeneratePartFilename(string title, int? partNo, int? partCount, string extension)
        {
            var prefix = string.Empty;
            // skládá se z jednotlivých dílů? přidám tedy do filename
            if ((partCount ?? 0) > 0)
            {
                var strPartNo = partCount > 99 ? $"{partNo:000}" : $"{partNo:00}";
                prefix = $"{strPartNo}-{partCount}-";
            }

            var filename = prefix + GetBaseFileName(title);
            filename = RemoveInvalidChars(filename.TrimToMaxLen(80)) + "." +extension;
            return filename;
        }
